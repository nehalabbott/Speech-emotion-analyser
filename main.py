...
# --- ADDED: IMPORTS FOR CONFUSION MATRIX ---
...
# --- ADDED ---
plot_confusion_matrix(y_test, y_pred_test, "Mixed Dataset (80/20 Test)", "cm_mixed.png")
...
# --- ADDED ---
plot_confusion_matrix(y_crema, y_pred_cre, "Train: RAVDESS -> Test: CREMA", "cm_rav_to_cre.png")
...
# --- ADDED ---
plot_confusion_matrix(y_ravdess, y_pred_rav, "Train: CREMA -> Test: RAVDESS", "cm_cre_to_rav.png")



# =========================================================================
# --- ADDED: TASK 3 POOLING EXPERIMENTS ---
# =========================================================================
print("\n" + "="*50)
print("STARTING TASK 3: POOLING METHOD EXPERIMENTS")
print("="*50)
...
def extract_raw_sequences(audio, sr):
    ...
    return np.vstack((mfcc, delta_mfcc, chroma, mel, rms))

def apply_pooling(seq, pool_type):
    if pool_type == 'max':
        return np.max(seq, axis=1)
    elif pool_type == 'mean+std':
        return np.hstack((np.mean(seq, axis=1), np.std(seq, axis=1)))

print("Extracting raw sequences for pooling tests...")
raw_X, raw_y = [], []

for root, _, files in os.walk(ravdess_path):
    for file in files:
        if file.endswith(".wav"):
            audio, sr = preprocess(os.path.join(root, file))
            if audio is not None:
                label = get_label_ravdess(file)
                if label:
                    raw_X.append(extract_raw_sequences(audio, sr))
                    raw_y.append(label)
...
for pool_type in ['max', 'mean+std']:
    print(f"\n--- Testing '{pool_type.upper()}' Pooling ---")
    X_pooled = np.array([apply_pooling(seq, pool_type) for seq in raw_X])
    X_tr, X_te, y_tr, y_te = train_test_split(X_pooled, raw_y, test_size=0.2, stratify=raw_y)
    
    sc = StandardScaler()
    X_tr_sc = sc.fit_transform(X_tr)
    X_te_sc = sc.transform(X_te)
    
    pool_model = SVC(kernel='rbf', C=5, gamma='scale', class_weight='balanced')
    pool_model.fit(X_tr_sc, y_tr)
    
    pool_preds = pool_model.predict(X_te_sc)
    print(f"Accuracy with {pool_type} pooling: {accuracy_score(y_te, pool_preds):.4f}")
    plot_confusion_matrix(y_te, pool_preds, f"Pooling: {pool_type.upper()}", f"cm_pool_{pool_type.replace('+', '_')}.png")